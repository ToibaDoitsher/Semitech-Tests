...
export default async function EditStudentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function updateStudent(formData: FormData) {
    ...
    const cohortLabel = String(formData.get("cohort_number") ?? "").trim();
    let cohort = await findCohortByLabel(sb, cohortLabel);
    if (!cohort) cohort = await createCohortByLabel(sb, cohortLabel);
  ...
  const cohortNum = enriched.cohorts?.number ?? enriched.cohort_name ?? "";

  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-semibold">עריכת תלמידה</h1>
      <p className="text-sm text-zinc-600">שכבה {formatCohortGradeLabel(enriched.grade_level)} · מחזור {cohortNum}</p>
      <form action={updateStudent} className="grid gap-3 rounded border bg-white p-4 md:grid-cols-2">
        ...
        <label className="block text-sm">מחזור<input name="cohort_number" type="number" defaultValue={String(cohortNum)} required className="mt-1 w-full border rounded px-2 py-1" /></label>
